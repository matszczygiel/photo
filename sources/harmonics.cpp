...
double Harmonics::NoNormCalcClmR( const int l, const int m, const int lx, const int ly, const int lz ) {
    // cartesian to spherical transformation coefficients r^l Y_{lm}
    ...
    if( m > 0 ) {
        ...
        for(int a = std::max(bs,0); a<=lima; a++) {
            twoa = 2 * a;
            faci = Const_arrays::binom[l][a] * Const_arrays::belt[a] * Const_arrays::fact[twol-twoa]
                    * Const_arrays::binom[a][bs] / Const_arrays::fact[l-am-twoa];
            for(int b=lim1b; b<=lim2b; b++) {
                twob = 2 * b + am - lx;
                if( twob % 2 == 1 ) continue;
                val += Const_arrays::binom[bs][b] * Const_arrays::binom[am][twob]
                        * Const_arrays::belt[twob/2] * faci;
            }
        }
