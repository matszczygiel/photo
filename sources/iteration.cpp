...
Iteration::status Iteration::one_step() {
	...
	for (unsigned i = 0; i < b_nk_l; i++)
		for (unsigned j = 0; j < b_nk_l; j++)
			for (unsigned a = 0; a < b_l; a++)
				for (unsigned b = 0; b < b_l; b++)
	...
	A_matrix_2eq = (norm_k_sqrt * H_non_k) + (H_matrix.topRows(b_nk_l) * Cont) * (Cont.adjoint() * S_matrix.leftCols(b_nk_l))
			+ (S_matrix.topRows(b_nk_l) * Cont) * (Cont.adjoint() * H_matrix.leftCols(b_nk_l)) + (H_kk * S_non_k) + k_R_k + kk_R;

	A_matrix_1eq_dum = H_matrix + (H_matrix * Ion) * (Ion.adjoint() * S_matrix) + (S_matrix * Ion) * (Ion.adjoint() * H_matrix) + H_pp * S_matrix
			+ p_R_p + pp_R;
	...
	S_matrix_2eq = norm_k_sqrt * S_non_k + (S_matrix.topRows(b_nk_l) * Cont) * (Cont.adjoint() * S_matrix.leftCols(b_nk_l));
